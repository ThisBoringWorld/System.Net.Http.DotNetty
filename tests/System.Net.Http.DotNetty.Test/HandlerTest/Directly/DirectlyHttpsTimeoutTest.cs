...
{
    [TestClass]
    public class DirectlyHttpsTimeoutTest
    {
        #region Private 字段
        ...
        public async Task HttpRequestTestAsync()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                tasks = Enumerable.Range(0, requestCount).Select(async m =>
                {
                    ...
                    try
                    {
                        var result = await _httpClient.PostAsync($"{TestServerConstant.EchoUrlHttps}?delay=2000", new ByteArrayContent(content), cts.Token);
                        Assert.Fail();
                    }
